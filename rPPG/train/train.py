...
def train(config: dict, videos: str, output: str, gt: str = None, saveStem: str = 'model', initModel: str = None, saveOptimizerState: bool = False, masks: str = None, cleanModels=False):
    """Train an rPPG model
    ...
    Arguments:
      ...
      masks: Path to masks
      cleanModels: If True, remove all intermediate models after training
    """
    import torch
    ...
    if cleanModels:
        for fname in outpath.iterdir():
            if fname.is_file() and not fname.suffix:
                fname.unlink()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cleanModels', action='store_true', help='After training, remove all intermediary models other than the one used for evaluation')
    ...
    train(config, args.videos, args.output, gt=args.gt, saveStem=args.saveStem, initModel=args.initModel, saveOptimizerState=args.saveOptimizerState, masks=args.masks, cleanModels=args.cleanModels)
